...
const AdminHome = () => {
  ...
  useEffect(() => {
    axiosSecure
      .get("/users")
      .then((res) => setAllUsers(res.data.totalCount))
      .catch((err) => console.log(err));
    axiosSecure
      .get("/donation-request-count")
      .then((res) => {
        setDonationRequest(res.data.totalCount);
      })
      .catch((err) => console.log(err));
  }, [axiosSecure]);
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 p-5">
        <div className="flex flex-col justify-center items-center py-3 bg-green-200 rounded-lg">
          ...
          <h1 className="text-lg font-medium">Total Users: {allUsers}</h1>
        </div>
        <div className="flex flex-col justify-center items-center py-3 bg-red-200 rounded-lg">
          ...
          <h1 className="text-lg font-medium text-center">
            Total Blood Donation <br /> Requests: {donationRequest}
          </h1>
        </div>
